    ...
    newParticipantBlock.addEventListener('contextmenu', (event) => {
        ...
        if (!contextMenu) {
            ...
            contextMenu.innerHTML = `
                ...
                <span class="context-menu-item" id="excludeHeroItem">Exclure des héros</span>
            `;
...
function showExcludeModal(targetBlock) {
    ...
    modalTitle.textContent = `Exclure des héros pour ${playerName}`;
...
function initializeModalButtons() {
    const modal = document.getElementById('excludeModal');
    if (modal) { 
        modal.querySelector('.modal-close-btn').addEventListener('click', closeExcludeModal);
